...
//!
//! Compiled only when the `debug` feature is enabled.
extern crate alloc;
use alloc::format;
...
pub fn dispatch(line: &str) {
    ...
    match parts.next() {
        ...
        Some(other) => crate::tty::serial_ldisc::write(
...
fn cmd_info(mut parts: SplitWhitespace) {
    match parts.next() {
        ...
        _ => crate::tty::serial_ldisc::write(b"usage: info mem | info proc\r\n"),
    }
}
...
fn cmd_info_mem() {
    ...
    crate::tty::serial_ldisc::write(
        format!("mem: used={used_kb} KiB  free={free_kb} KiB  total={total_kb} KiB\r\n").as_bytes(),
    );
}

fn cmd_info_proc() {
    crate::tty::serial_ldisc::write(b"PID   PPID  STATE       NAME\r\n");
...
fn cmd_bt() {
    ...
    while fp != 0 && depth < 32 {
        ...
        crate::tty::serial_ldisc::write(format!("  #{depth:02}  0x{ra:016x}\r\n").as_bytes());
        if pfp == fp {
            break;
        }
        fp = pfp;
...
fn cmd_dump(mut parts: SplitWhitespace) {
    ...
    let Some(addr) = addr else {
        crate::tty::serial_ldisc::write(b"usage: dump <hex_addr> [len]\r\n");
        return;
    };
    ...
    for row in 0..rows {
        ...
        crate::tty::serial_ldisc::write(line.as_bytes());
    }
}
