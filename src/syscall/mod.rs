...
//!
//! ## Recently wired
//!   NR 425  io_uring_setup(entries, params)    =>
//! io_uring::syscall::sys_io_uring_setup   NR 426  io_uring_enter(fd, …)
//! => io_uring::syscall::sys_io_uring_enter   NR 427  io_uring_register(fd, op,
//! arg, n)  => io_uring::syscall::sys_io_uring_register   NR 41-55 socket
//! syscalls (all 15)   NR 288   accept4
//!   NR 318  getrandom(buf, count, flags)     => stubs::sys_getrandom_impl
//!   NR 334  close_range(first, last, flags)  =>
//! fs::close_range::sys_close_range   NR 332  statx
//! => posix_full::sys_statx_impl   NR 326  copy_file_range                  =>
//! posix_full::sys_copy_file_range_impl   NR 435  clone3
//! => proc::clone::sys_clone3   NR 437  openat2                          =>
//! openat2_mincore::sys_openat2_impl   0x8000_0000  kmtest_list
...
pub fn dispatch_with_rip(
    ...
    let ret;
    if let Some(r) = routers::dispatch_filesystem(&ctx) {
        ret = r;
    } else if let Some(r) = routers::dispatch_process(&ctx) {
        ret = r;
    } else if let Some(r) = routers::dispatch_memory(&ctx) {
        ret = r;
...
pub(crate) fn shmctl_dispatch(shmid: i32, cmd: i32, buf_va: usize) -> isize {
    if cmd == crate::ipc::IPC_SET {
        ...
        let mut buf = [0u8; SZ];
        if crate::uaccess::copy_from_user(&mut buf, buf_va).is_err() {
            return efault();
        }
        match parse_shmid_ds(&buf) {
            Some(ds) => match shm::shmctl_set(shmid, ds) {
                Ok(()) => 0,
                Err(e) => e,
            },
            None => efault(),
...
pub(crate) fn semop_dispatch(semid: i32, sops_va: usize, nsops: usize) -> isize {
    match copy_sembuf_from_user(sops_va, nsops) {
        Some(ops) => match sem::semop(semid, &ops) {
            Ok(()) => 0,
            Err(e) => e,
        },
        None => efault(),
...
pub(crate) fn msgsnd_dispatch(msqid: i32, msgp_va: usize, msgsz: usize, msgflg: i32) -> isize {
    match copy_msgbuf_from_user(msgp_va, msgsz) {
        Some((mtype, data)) => match msg::msgsnd(msqid, mtype, data, msgflg) {
            Ok(()) => 0,
            Err(e) => e,
        },
        None => efault(),
...
pub(crate) fn msgrcv_dispatch(
    ...
    match msg::msgrcv(msqid, msgsz, msgtyp, msgflg) {
        Ok((mtype, data)) => {
            if !copy_msgbuf_to_user(msgp_va, mtype, &data) {
                return efault();
            }
            data.len() as isize
        },
...
pub(crate) fn msgctl_dispatch(msqid: i32, cmd: i32, buf_va: usize) -> isize {
    if cmd == crate::ipc::IPC_SET {
        ...
        match parse_msqid_ds(&buf) {
            Some(ds) => match msg::msgctl_set(msqid, ds) {
                Ok(()) => 0,
                Err(e) => e,
            },
            None => efault(),
...
pub(crate) fn mq_open_dispatch(name_va: usize, oflag: i32, mode: u32, attr_va: usize) -> isize {
    match crate::proc::exec::read_cstr_safe(name_va) {
        Some(name) => {
            let attr = if attr_va != 0 {
                copy_mq_attr_from_user(attr_va)
            } else {
                None
            };
...
pub(crate) fn mq_unlink_dispatch(name_va: usize) -> isize {
    match crate::proc::exec::read_cstr_safe(name_va) {
        Some(name) => match mq::mq_unlink(&name) {
            Ok(()) => 0,
            Err(e) => e,
        },
        None => efault(),
...
pub(crate) fn mq_timedsend_dispatch(mqd: u64, msg_va: usize, msglen: usize, prio: u32) -> isize {
    ...
    if crate::uaccess::copy_from_user(&mut buf, msg_va).is_err() {
        return efault();
    }
    match mq::mq_send(mqd, buf, prio) {
        Ok(()) => 0,
        Err(e) => e,
    }
}

pub(crate) fn mq_timedreceive_dispatch(
    mqd: u64,
    buf_va: usize,
    buflen: usize,
    prio_va: usize,
    ...
    if new_va != 0 {
        match copy_mq_attr_from_user(new_va) {
            Some(new) => match mq::mq_setattr(mqd, new) {
                Ok(old) => {
                    if old_va != 0 && !copy_mq_attr_to_user(old_va, &old) {
                        return efault();
                    }
                    0
                },
        ...
        match mq::mq_getattr(mqd) {
            Ok(attr) => {
                if old_va != 0 && !copy_mq_attr_to_user(old_va, &attr) {
                    return efault();
                }
                0
            },
