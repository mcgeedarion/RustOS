...
/// alias of `with_procs_ro` — callers use `scheduler::with_procs`.
#[inline]
...
/// pidfd.rs uses imperative `procs_lock`/`procs_unlock` pairs. We
...
/// no-op pair — the snapshot from `procs_lock` releases via Drop.
#[inline]
pub fn procs_unlock() {}
