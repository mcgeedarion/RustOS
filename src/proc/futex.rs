...
//!
//! ## Ops implemented
//!   FUTEX_WAIT           (0)  — sleep if *uaddr == val; optional timespec
//! timeout   FUTEX_WAKE           (1)  — wake up to val waiters
...
fn pi_boost(mut owner_pid: usize, waiter_pid: usize) {
    ...
    for _ in 0..PI_CHAIN_LIMIT {
        let (already_highest, _) = scheduler::with_proc_mut(owner_pid, |pcb, _pl| {
...
pub fn futex_wake_bitset(addr: usize, count: usize, mask: u32) -> usize {
    ...
    let (to_wake, wq): (Vec<usize>, Option<Arc<WaitQueue>>) = {
        let mut tbl = FUTEX_TABLE.lock();
        let bucket = match tbl.get_mut(&key) {
            Some(b) => b,
            None => return 0,
        };
        ...
        let pids: Vec<usize> = indices
            .iter()
            .rev()
            .map(|&i| bucket.waiters.remove(i).pid)
            .collect();
        let wq_clone = if !pids.is_empty() {
            Some(bucket.wq.clone())
        } else {
            None
        };
        if bucket.waiters.is_empty() {
...
fn futex_requeue_inner(src: FutexKey, dst: FutexKey, requeue_count: usize) -> usize {
    ...
    let to_move: Vec<Waiter> = {
        let src_bucket = match tbl.get_mut(&src) {
            Some(b) => b,
            None => return 0,
        };
    ...
    if n > 0 {
        tbl.entry(dst)
            .or_insert_with(FutexBucket::new)
            .waiters
            .extend(to_move);
    }
