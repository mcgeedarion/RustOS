...
// ---------------------------------------------------------------------------
// Helpers extracted from sys_clone3 to satisfy clippy::cognitive_complexity.
// ---------------------------------------------------------------------------

/// Build the arch-specific kernel context, first PC, and first SP for a
/// newly cloned child.  Called once per clone3() invocation, before enqueue.
#[cfg(target_arch = "x86_64")]
...
fn build_child_context(
    ...
    push_trap_frame_riscv(kstack_top, parent_pc, effective_sp, child_tls);
    ...
    (ctx, parent_pc, effective_sp)
}

/// Write TID values into userspace and return the child_tid_va for the PCB.
fn apply_tid_flags(ca: &CloneArgs, flags: u64, child_pid: usize) -> usize {
...
pub fn sys_clone3(args_va: usize, args_size: usize) -> isize {
    ...
    // Build the arch-specific kernel context and capture first PC/SP.
    // pc / sp MUST be set before enqueue() so Task::new() captures them.
    let (child_ctx, child_first_pc, child_first_sp) =
        build_child_context(kstack_top, parent_pc, user_sp, child_tls);

    // Write TID values into userspace and derive child_tid_va for the PCB.
    let child_tid_va = apply_tid_flags(&ca, flags, child_pid);
