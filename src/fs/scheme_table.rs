...
///
/// Methods that a handler does not support may return
/// `Err(SchemeError::InvalidArg)` or `Err(SchemeError::Other)`.  The default
/// implementations below do exactly that so that minimal handlers only need to
/// implement `open`, `read`, and `close`.
pub trait Scheme: Send + Sync {
    ...
    ///
    /// `whence` follows POSIX semantics: 0 = SEEK_SET, 1 = SEEK_CUR, 2 =
    /// SEEK_END.
...
impl SchemeTable {
    ...
    ///
    /// `name` must not include the trailing colon (e.g., `"tcp"`, not
    /// `"tcp:"`). Re-registering an existing name overwrites the previous
    /// handler.
