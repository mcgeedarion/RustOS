...
impl Bpb {
    ...
    pub fn data_clusters(&self) -> u32 {
        let fat_sectors = self.num_fats as u64 * self.fat_size_32 as u64;
        let data_sectors =
            self.total_sectors_32 as u64 - self.reserved_sectors as u64 - fat_sectors;
...
impl Fat32Fs {
    ...
    /// Walk the cluster chain starting at `start`, collecting all cluster
    /// numbers.
    pub fn cluster_chain(&self, start: u32) -> Result<Vec<u32>, isize> {
        let mut chain = Vec::new();
        let mut cur = start;
        loop {
            if cur < 2 || cur >= FAT32_BAD {
                break;
            }
            chain.push(cur);
    ...
    /// Allocate a new cluster, link it after `prev` (or leave unlinked if
    /// None).
            ...
            if self.fat_get(c)? == FAT32_FREE {
                ...
                for s in 0..self.bpb.sectors_per_cluster as u64 {
                    blk_write(
                        self.dev,
                        lba + s,
                        &zero[s as usize * 512..(s as usize + 1) * 512],
                    )?;
                }
                return Ok(c);
            }
    ...
    fn read_cluster(&self, cluster: u32, buf: &mut [u8]) -> Result<(), isize> {
        ...
        for s in 0..spc {
            blk_read(
                self.dev,
                lba + s as u64,
                &mut buf[s * SECTOR_SIZE..(s + 1) * SECTOR_SIZE],
            )?;
        }
        Ok(())
    }
    ...
    fn write_cluster(&self, cluster: u32, buf: &[u8]) -> Result<(), isize> {
        ...
        for s in 0..spc {
            blk_write(
                self.dev,
                lba + s as u64,
                &buf[s * SECTOR_SIZE..(s + 1) * SECTOR_SIZE],
            )?;
        }
        Ok(())
    }

    /// Read all valid directory entries from the cluster chain rooted at
    /// `dir_cluster`. LFN sequences are collected and applied to the
    ...
    pub fn read_dir(&self, dir_cluster: u32) -> Result<Vec<DirEntry>, isize> {
        ...
        for cluster in chain {
            ...
            while off + 32 <= buf.len() {
                ...
                // Short entry
                let attr = e[11];
                if attr & ATTR_VOLUME_ID != 0 && attr != ATTR_LFN {
                    lfn_buf.clear();
                    off += 32;
                    continue;
                }
    ...
    /// Look up a single name in a directory cluster.  Returns the DirEntry or
    /// Err(-2).
    ...
    pub fn statfs(&self) -> FatStatfs {
        let total = self.bpb.data_clusters();
        let free = if self.free_count != u32::MAX {
            self.free_count
        } else {
            0
        };
