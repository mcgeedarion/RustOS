...
pub mod proc {
    pub mod exec {
        ...
        pub fn spawn_user_process_from_bytes(
            ...
            crate::serial_println!("aarch64-userspace: spawn begin");
            match prepare_user_process_from_bytes(path, elf, argv, envp) {
                Ok(mut task) => {
                    #[cfg(target_arch = "aarch64")]
                    crate::serial_println!("aarch64-userspace: task image prepared");
                    #[cfg(target_arch = "aarch64")]
                    crate::serial_println!("userspace: prepared PID 1 /init");
                ...
                Err(reason) => {
                    ...
                    {
                        let _ = (path, reason);
                        crate::serial_println!("aarch64-userspace: spawn rejected");
                        crate::serial_println!("userspace: rejected /init");
                    }
                    #[cfg(not(target_arch = "aarch64"))]
        ...
        fn prepare_user_process_from_bytes(
            ...
            #[cfg(target_arch = "aarch64")]
            crate::serial_println!("aarch64-userspace: parse elf");
            let loaded = parse_elf64(elf)?;
            #[cfg(target_arch = "aarch64")]
            crate::serial_println!("aarch64-userspace: elf parsed");
            ...
            #[cfg(target_arch = "aarch64")]
            crate::serial_println!("aarch64-userspace: load mappings planned");
            ...
            #[cfg(target_arch = "aarch64")]
            crate::serial_println!("aarch64-userspace: mappings planned");
            let user_sp = build_initial_stack_pointer(argv, envp)?;
            #[cfg(target_arch = "aarch64")]
            crate::serial_println!("aarch64-userspace: task image ready");
            Ok(UserTaskImage {
                pid: INIT_PID,
                ppid: INIT_PPID,
        ...
        #[cfg(target_arch = "aarch64")]
        fn finalize_and_enter_user(task: &mut UserTaskImage, elf: &[u8]) -> bool {
            crate::serial_println!("aarch64-userspace: build address space");
            let ttbr0 =
                match aarch64_user_entry::build_address_space(&task.address_space.mappings, elf) {
                    Ok(ttbr0) => ttbr0,
                    Err(reason) => {
                        let _ = reason;
                        crate::serial_println!("aarch64-userspace: address space failed");
                        crate::serial_println!("userspace: transition blocked");
                        return false;
                    },
                };
            crate::serial_println!("aarch64-userspace: address space built");
            task.address_space.cr3 = Some(ttbr0);
            crate::serial_println!("aarch64-userspace: eret to EL0");
            crate::serial_println!("userspace: entering EL0");
