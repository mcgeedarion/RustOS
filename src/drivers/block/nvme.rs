...
fn io_rw(nsid: u32, lba: u64, count: u32, buf_phys: u64, write: bool) -> Result<(), &'static str> {
    if count == 0 {
    ...
    // Set up PRPs; requires controller lock to access prp_list_phys.
    {
        ...
        let (prp1, prp2) = unsafe { build_prps(buf_phys, byte_len, c.prp_list_phys)? };
        cmd.prp1 = prp1;
        cmd.prp2 = prp2;
    }
    ...
    unsafe {
        write_volatile((c.bar0 as usize + sq_db) as *mut u32, c.iosq_tail as u32);
    }
    ...
    for _ in 0..timeout {
        ...
        let cqe = unsafe { &*((c.iocq_phys as usize + c.iocq_head * 16) as *const CqEntry) };
        ...
        if (cqe.status & 1) as u8 == c.iocq_phase {
            ...
            unsafe {
                write_volatile((c.bar0 as usize + cq_db) as *mut u32, c.iocq_head as u32);
            }
            return if sc == 0 {
                Ok(())
            } else {
...
fn admin_submit(cmd: SqEntry) {
    ...
    unsafe {
        write_volatile((c.bar0 as usize + 0x1000) as *mut u32, c.asq_tail as u32);
    }
}
...
fn admin_complete() -> Result<(), &'static str> {
    ...
    for _ in 0..timeout {
        ...
        let cqe = unsafe { &*((c.acq_phys as usize + c.acq_head * 16) as *const CqEntry) };
        fence(Ordering::Acquire);
        ...
        if (cqe.status & 1) as u8 == c.acq_phase {
            ...
            unsafe {
                write_volatile((c.bar0 as usize + db) as *mut u32, c.acq_head as u32);
            }
            return if sc == 0 {
                Ok(())
            } else {
