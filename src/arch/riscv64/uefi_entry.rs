//! RISC-V 64 UEFI firmware entry point — `efi_main`.
//!
//! ## Boot priority: TERTIARY (riscv64 — experimental boot target)
//!
//! The UEFI firmware calls `efi_main(image_handle, system_table)` with
//! interrupts disabled and a flat identity-mapped address space.
//!
//! ## What we do here
...
unsafe extern "efiapi" fn efi_main(
    ...
    // 1. Banner — identifies this as the TERTIARY (riscv64) boot target.
    efi_print(
        st.con_out,
        "RustOS [TERTIARY] riscv64 booting via UEFI...\r\n",
    );
    ...
    unsafe {
        crate::arch::riscv64::serial::init();
    }

    // 7. Switch to kernel boot stack and tail-call kernel_main.
    #[cfg(not(any(feature = "boot_minimal", feature = "userspace_boot")))]
    crate::arch::riscv64::hal::early_init();
    kernel_main_jump(&raw const BOOT_INFO);
}
...
/// then branch to the common entry.
#[inline(never)]
...
unsafe fn efi_print(con_out: *mut EfiSimpleTextOutput, msg: &str) {
    ...
    wide[len] = 0;
    let _ = ((*con_out).output_string)(con_out, wide.as_ptr());
}
