...
use crate::proc::{
    ...
    signal::{send_signal, sys_rt_sigaction},
    wait::sys_waitpid,
};
...
fn proc_fork_multiple() -> KmTestResult {
    for expected in [11u8, 22u8] {
        let pid = sys_fork();
        if pid < 0 {
            return Err("fork() failed in multi-fork test");
        }
        if pid == 0 {
        ...
        if w != pid as isize {
            return Err("waitpid wrong pid (multi-fork)");
        }
        let code = (status >> 8) & 0xFF;
        if code != expected as i32 {
            return Err("multi-fork exit code mismatch");
        }
    }
    Ok(())
}
...
fn proc_waitpid_wnohang() -> KmTestResult {
    let pid = sys_fork();
    if pid < 0 {
        return Err("fork failed (wnohang)");
    }
    if pid == 0 {
    ...
    if w < 0 {
        return Err("waitpid WNOHANG returned error");
    }
...
fn proc_signal_self_ignore() -> KmTestResult {
    ...
    let r = sys_rt_sigaction(SIGUSR1, sigaction_buf.as_ptr() as usize, 0, 8);
    if r != 0 {
        return Err("rt_sigaction SIG_IGN failed");
    }
...
fn proc_getppid_correct() -> KmTestResult {
    ...
    let pid = sys_fork();
    if pid < 0 {
        return Err("fork failed (getppid)");
    }
    if pid == 0 {
